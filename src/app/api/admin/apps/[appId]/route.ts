...
const unauthorized = (): NextResponse =>
  NextResponse.json(
    { error: { code: 'UNAUTHORIZED', message: 'Sign in to manage apps.' } },
...
export async function GET(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const safeAppID = normalizeConsoleAppIdentifier(appId)
  const workspaceID = normalizeConsoleAppIdentifier(
    new URL(request.url).searchParams.get('workspaceId'),
  )
  if (!safeAppID || !workspaceID) {
    return NextResponse.json(
      { error: { code: 'INVALID_INPUT', message: 'Select a valid app and workspace.' } },
      { headers: noStoreHeaders, status: 400 },
    )
  }

  return resultResponse(await getConsoleAppDetail(payload, auth.user, safeAppID, workspaceID))
}
...
export async function PATCH(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  if (!safeAppID || !body.ok) {
    const message = body.ok ? 'Select a valid app.' : body.message
  ...
  const input = parseAppConsoleMutation(body.value)
  if (!input.ok) {
    return NextResponse.json(
  ...
  return resultResponse(await mutateConsoleApp(payload, auth.user, safeAppID, input.value))
}
