...
export default async function SharedFallbackPage({
  params,
}: {
  params: Promise<{ linkSlug: string; publicAppKey: string }>
}) {
  const { linkSlug, publicAppKey } = await params
  const requestHeaders = new Headers()
  ;(await headers()).forEach((value, key) => requestHeaders.set(key, value))
  const host = await resolvePublicHost(
    new Request('http://localhost/shared-public-link-page', { headers: requestHeaders }),
    'resolver',
  )
  if (!host.ok || host.kind !== 'shared') notFound()

  return (
    <FallbackConnector
      appSlug={publicAppKey}
      linkSlug={linkSlug}
