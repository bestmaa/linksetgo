...
Relay Community runs as one Next.js/Payload application and one PostgreSQL
...
both images, runs the migration service, and starts Relay. It never prints the
generated secrets. The published application port binds to `127.0.0.1` by
default, so a reverse proxy on the same host can reach Relay without exposing the
container port directly to the network.

For an HTTPS installation the helper requires `RELAY_SOURCE_URL`. It stores that
exact credential-free HTTPS repository or archive as `SOURCE_CODE_URL` and Relay
...
tunnel, and create the first owner. Payload promotes that user to Relay super
...
HTTPS donation page; when it is blank or invalid, Relay keeps sponsorship disabled
...
`NEXT_PUBLIC_SITE_URL` supplies build-time marketing metadata, so rebuild Relay
...
- use a real client IP only for proxy controls, without forwarding or logging it
  unnecessarily in Relay;
- limit `/cms` by VPN, SSO-aware proxy, or IP allowlist;
- preserve Relay's Content Security Policy unless a replacement has been tested
  against both the Relay console and Payload emergency CMS.

Relay sets a conservative Content Security Policy plus HSTS, frame,
...
Monitor the byte size and status of both mobile trust files. Relay paginates all
...
PostgreSQL disk alerts, and CDN/WAF rate limits. Relay's in-database event
